...
def scDHA_class(path_name, latent: np.ndarray, n_jobs=-1, seed=None):
    """This method does classification on the dataset

    Args:
        data (string): user passed latent variable
        path_name (string): the name of the dataset
        n_jobs (int, optional): number of cpu cores used in knn. Defaults to -1 (use all cores).
        seed ([int], optional): random number seed, so result can be reproduced. Defaults to None.
...
def scDHA_clus(path_name, n_clusters):
    data = np.load(f'latent/{path_name}.n   py')
    ...
    for latent in data:
        ...
        silhouette = silhouette_score(latent, cluster)
        if ari > best_ari:
            best_latent = latent
            best_ari = ari
...
def scDHA_clus(path_name, data: np.ndarray, n_clusters):
    ...
    print(f'ari: {best_ari}')
    print(f'silhouette score: {best_silhouette}')
    return best_latent, best_cluster, best_ari, best_silhouette
...
def scDHA_vis(path_name, best_latent, best_cluster, method='umap'):
    if method != 'umap':
        print('method can only be umap because I only implemented this, using umap instead')
